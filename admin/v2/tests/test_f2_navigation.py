...
try:
    with sync_playwright() as p:
        ...
        for rt in sorted(expected):
            ...
            st=pg.evaluate("""(want)=>{const v=document.querySelector('#view').innerText;
              return {nf:v.includes(want.nf), badge:v.includes(want.badge),
                ...
                {"nf":NF,"badge":BADGE})
            if st["nf"] or (rt not in ("overview","users","ai-providers","ai-route","integrations-n8n","integrations-metaapi","integrations-email","ai-health","system-health","system-logs","security-audit","system-flags","analytics-users","analytics-trading","analytics-ai","analytics-revenue","billing-overview") and not st["badge"]):
        ...
        wbad=[]
        IMPL={"overview","users","ai-providers","ai-route","integrations-n8n","integrations-metaapi","integrations-email","ai-health","system-health","system-logs","security-audit","system-flags","analytics-users","analytics-trading","analytics-ai","analytics-revenue","billing-overview"}
        for rt in sorted(expected):
