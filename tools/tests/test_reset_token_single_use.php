...
/**
 * TEST-04 — A reset token is strictly single-use.
 *
 * Flow: mint token -> successful reset -> attempt the SAME token again.
 * Expected: the second attempt is rejected AND the password does NOT change again.
 *
 * Deterministic: temp SQLite + MAIL_DRIVER=log. No provider, no network, no secrets.
 */

$root = sys_get_temp_dir() . '/velora-reset-single-use-' . bin2hex(random_bytes(5));
...
try {
    ...
    $pdo->prepare('INSERT INTO users (email,password_hash,full_name,email_verified_at) VALUES (?,?,?,?)')
        ->execute([$email, password_hash('OldPassword123!', PASSWORD_BCRYPT), 'Single Use', gmdate('Y-m-d H:i:s')]);
    ...
    $service = new Velora\Auth\PasswordService();
    $service->resetPassword(['token' => $token, 'newPassword' => 'NewPassword456!']);
    $hashAfterFirst = (string) $pdo->query('SELECT password_hash FROM users WHERE id=' . $userId)->fetchColumn();

    $secondRejected = false;
    try {
        $service->resetPassword(['token' => $token, 'newPassword' => 'AnotherPassword789!']);
    } catch (Velora\Core\Exceptions\ValidationException) {
        $secondRejected = true;
    }
    $expect($secondRejected, 'a consumed reset token must be rejected on reuse');

    $hashAfterSecond = (string) $pdo->query('SELECT password_hash FROM users WHERE id=' . $userId)->fetchColumn();
    $expect($hashAfterSecond === $hashAfterFirst, 'password hash must not change on token reuse');
    $expect(!password_verify('AnotherPassword789!', $hashAfterSecond), 'rejected reuse must not authenticate the attacker-chosen password');
    $expect(
        (int) $pdo->query('SELECT used_at IS NOT NULL FROM password_resets WHERE user_id=' . $userId)->fetchColumn() === 1,
        'reset row must stay consumed',
    );

    echo "TEST-04 reset token single-use: PASS ({$assertions} assertions)\n";
} finally {
