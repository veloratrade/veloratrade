...
- ✉️ **مهاجرت Resend فقط روی Staging — تحویل موفق، E2E نیازمند follow-up:** Transport `resend` با PHP cURL و Secret مستقل `RESEND_API_KEY` پیاده‌سازی شد؛ هیچ Composer dependency اضافه نشد و From در کد به `VELORA TRADE <no-reply@veloratrade.ir>` محدود است. Setup Staging run `32268203145` ✅، Deploy Staging run `32268282263` ✅، Resend Diagnostics run `32270666565` ✅: API پذیرفت، Message ID امن `56b3b612-82bd-4e99-809d-e4c83e33007b` برگشت، تحویل خارجی در ۱۰ ثانیه و From دقیق تأیید شد. Register E2E run `32270958783`: register=201 ✅، گیت EMAIL_NOT_VERIFIED ✅، دریافت ایمیل و token در ۱۲ ثانیه ✅، اما `verify-email` با HTTP 500 ❌؛ این خطا پس از تحویل موفق و مستقل از Transport است. Health Check Staging run `32273336725` ✅. بررسی مستقیم `email_notifications` در run `32271490231` به‌علت ناپایداری FTP کامل نشد و باید با دسترسی خواندنی مطمئن تکرار شود. Production دست‌نخورده است.
