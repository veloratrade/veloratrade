...
| OC-13 | **دورزدنِ گارد با sparse روی کلونِ کامل + عمل پیش از خواندنِ قرارداد.** یک جلسه کلونِ کامل زد، سپس روی همان `.git`ِ کامل `sparse-checkout` زد تا گارد «سبز» شود؛ در حالی که تاریخچهٔ کامل (۴۷مگ/۲۱۶ کامیت) و PAT داخل `.git/config` باقی ماند. همچنین کلون را پیش از خواندنِ AGENTS.md زد | گارد باید کلونِ کامل را **مستقل از پرچمِ sparse** بشناسد: not-shallow + commits>5 + حجمِ .git + credential در `.git/config` = نقضِ HARD؛ کدِ `VELORA-RUN` در صورت نقض چاپ نشود؛ نخستین واکشیِ هر جلسه فقط تک‌فایلِ AGENTS.md است («هنوز نخوانده بودم» عذر نیست). ابزارِ اجرا: `tools/velora-bootstrap.sh` |
| OC-14 | **فرضِ مکانِ سورس بدونِ structure baseline باطل است.** یک جلسه فایل‌هایِ `_next/` (که legacy/inactive‌اند) را فرانت‌eadِ واقعی پنداشت و چون `package.json`/`.tsx` وجود نداشت، اعلام کرد «سورسِ فرانت‌ead موجود نیست/غیرقابل‌تعمیر» — در حالی که پروژه vanilla JS + HTML است و سورس (قالب‌هایِ ریشهٔ مخزن + `public/assets/*.js`) در همان مخزن حضور داشت | قبل از هر نتیجه‌گیریِ «قابل‌تعمیر نیست/سورس نیست»: (۱) مکان را از `docs/03_PROJECT_STRUCTURE_BASELINE.md` (§3 و §7) بیاب، (۲) وجودِ فیزیکیِ فایل را راستی‌آزمایی کن. نبودِ فایل‌هایِ framework برایِ پروژهٔ بدون-bundler دلیلِ «نبودِ سورس» نیست. تست: `tools/tests/test_structure_source_locator.py` |

## 7. وضعیت جاری و کارهای باز (Living Section)

**آخرین وضعیت (2026-08-20 — جلسهٔ agent):**
- 🧠 **Feature جدید Dashboard AI Insights روی شاخهٔ کاری Agent پیاده شد (هنوز commit/push نشده):** در `dashboard/index.html` یک تب/section جدید با namespace `dashboard.aiInsights.*` اضافه شد؛ title/empty/loading/error/action stateها همگی به‌صورت دوزبانه در `public/locales/{fa,en}.json` ثبت شدند و هیچ متن کاربری hardcoded جدیدی وارد UI نشد. UI از همان مکانیزم‌های موجود (`data-i18n` + `t()/errorMessage()` + build localized) استفاده می‌کند و action اصلی کاربر را به مسیر موجود `intelligence` می‌برد؛ endpoint یا helper جدیدی ساخته نشد. build localized با release `2026.08.24.2` بازتولید شد و گیت‌های `validate_localization`, `check_hardcoded_ui`, `check_frozen_hash_keys`, anomaly/orphan و `build_localized_static.py --check` همگی PASS هستند. منتظر تصمیم مالک برای commit/push.
- 🌍 **سخت‌سازی i18n برای توسعهٔ آینده روی شاخهٔ کاری Agent اعمال شد (بدون تغییر Production/Staging و بدون اجرای Actions):** `CLAUDE.md` ریشه به‌عنوان قرارداد مشارکت‌کننده افزوده شد؛ همان validatorهای موجودِ `report_catalog_anomalies.py` و `report_orphan_catalog_keys.py` با allowlist دقیق baseline به حالت blocking ارتقا یافتند تا CI روی کلیدهای orphan جدید، ترجمهٔ EN خالیِ جدید، و `FA==EN`های ترجمه‌نشده fail کند، درحالی‌که parity/placeholder mismatch از قبل blocking بودند. همچنین **همان checker موجودِ** `check_hardcoded_ui.py` بدون ساخت checker دوم، از freeze صرفِ literalهای فارسی به freeze دوزبانه ارتقا یافت: حالا literalهای سخت‌کدشدهٔ UI در **فارسی و انگلیسی** را می‌گیرد و برای literalهای مجازِ technical allowlist دقیقِ literal-level دارد. در audit نهایی integrity، ruleهای broad/misclassified و stale از `tools/localization/allowlist-hardcoded.json` حذف/نظیف شدند، برای هر rule دلیل و منبع ثبت شد، و baseline فقط به literalهای واقعیِ باقی‌مانده کاهش یافت. استثناهای فعلی به‌صورت exact-key در `tools/localization/catalog-quality-allowlist.json` و exact/regex در `tools/localization/allowlist-hardcoded.json` ثبت شدند و stale allowlist نیز خودش fail می‌شود. این تغییر فقط hardening است؛ معماری i18n موجود refactor نشد.
- 🚀 **سیستم هفت آیکون اختصاصی CID روی Production منتشر و Live-verified شد:** Dry-run `32318428446` ✅ (گیت Staging/CSP/security/backup، صفر نوشتن)، Deploy Production `32319549527` ✅ (Backup Artifact + upload + read-only Health Check). تست مستقیم خارج از Actions: register=201، تحویل ۱۰ ثانیه، `verification.png` به‌صورت inline/related CID، From صحیح، Gold Outline، لوگوی Production HTTP 200/image/png، نبود Emoji و helper link، صفحه=200، verify API=200، token پاک، UI موفق و login=200؛ `PRODUCTION_CID_EMAIL=PASS`. Actions پس از تست دوباره غیرفعال است.
- 🛡️ **هفت آیکون اختصاصی CID روی Staging منتشر و 7/7 Live-verified شدند:** آیکون‌های verification/welcome/password-reset/password-changed/security/first-trade/achievement از SVG منبع به PNG شفاف ۱۲۸×۱۲۸ تبدیل، Emojiهای قدیمی حذف و با Resend `content_id` داخل ایمیل Embed شدند (`33b9b8a`). Deploy Staging `32315509808` ✅؛ هر هفت Asset HTTP 200/image/png. Suite نهایی `32317939080` با چهار صندوق کم‌حجم: `auth 2/2`، `password 2/2`، `security 1/1`، `activity 2/2` و نتیجه `TRANSACTIONAL_CID_DELIVERIES=7/7 PASS`؛ From، Gold Outline، لوگوی محیط، inline/related attachment و نبود Emoji همگی تأیید شدند. شکست‌های اولیه 6/7 و HTTP 429 مربوط به burst/rate-limit سرویس mail.tm بود و با تست `0d29f3c` رفع شد. Actions پس از تست دوباره غیرفعال است.
- 🎨 **قالب منتخب Gold Outline روی Staging منتشر و Live-verified شد؛ Production هنوز تغییر نکرده:** انتخاب مالک `Midnight Gold Classic + Gold Outline` در `api/src/Core/EmailTemplate.php` پیاده‌سازی شد (`4ef2432` + fix محیط‌محور لوگو `617213b`): کارت table-based با قاب طلایی دولایه، هدر/فوتر ثابت، CTA اصلی بدون لینک کمکی و لوگوی واقعی PNG. Deploy Staging `32312976423` ✅. تست ایمیل واقعی: تحویل ۱۰ ثانیه، From صحیح، outer `#9b782e`، inner `#3b465a`، لوگوی Staging HTTP 200/image/png، نبود helper link و CTA اصلی همگی PASS. قرارداد قالب `22/22` و Resend mock `26/26` سبز است. Actions پس از تست دوباره غیرفعال شد؛ Production همچنان قالب قبلی را دارد.
...
- 🧭 **قاعدهٔ منبع حقیقت زندهٔ n8n (۲۰۲۶-۰۸-۲۸ — جلسهٔ agent):** درس جلسه: استنتاجِ «خالی بودن `content/n8n-archive/` ⟹ آرشیو وجود ندارد» ممنوع شد؛ برای هر وضعیت زندهٔ مرتبط با n8n، اینستنس زندهٔ SOURCE منبع حقیقت است. ثبت: AGENTS.md §2.4 (قانون صریح + اولویت `Live SOURCE ⬇ Snapshot ⬇ درخواست ورودی` + محدودهٔ read-only) و افزوده در §2.1/§11 همان فایل، BR-11 در §8 همین سند، و اشارهٔ متقابل در `docs/N8N_ARCHIVE_AGENT.md` و `content/n8n-archive/README.md`. گیت صلاحیت پردازش snapshot-محور تغییر نکرد. فقط docs؛ بدون تغییر کد/محیط/Deploy.
...
> ۳. توکنِ داخلِ چت تا پایانِ پروژهٔ جاری معتبر و قابلِ استفادهٔ مجدد است (ادمین نیاز به ارسالِ مجدد ندارد)؛ در پایانِ پروژه **rotate شود** (B-5).
>
...
#### قالبِ پیشنهادیِ دستورِ مالک (برای هر چتِ جدید)

این قالب حلقهٔ مرغ‌تخم‌مرغی را می‌بندد: ایجنت تا خواندنِ AGENTS.md نمی‌داند نباید کلون کند،
پس خودِ نخستین دستور باید آن را الزام کند. در هر چتِ جدید یکسان Paste شود:

```
مخزن: Veloratrade/veloratrade (عمومی).
اول فقط tools/velora-bootstrap.sh (یا AGENTS.md) را تک‌فایل از API واکشی کن و اجرا کن.
هیچ کلونِ کاملی نزن؛ برای هر مأموریت فقط فایل‌های همان قسمت را sparse بیاور و پس از کار پاک کن.
خواندن ناشناس است؛ برای push از deploy key استفاده کن و هرگز credential را در URL کلون نگذار.
```
...
| BR-8 | **عمل پیش از خواندنِ قرارداد ممنوع؛ وقتی گارد پرچم زد به هدفِ قانون عمل کن.** نخستین واکشیِ هر جلسه فقط تک‌فایلِ AGENTS.md است؛ کلونِ کامل ممنوع. در صورت نقض، به جای «عبور از چک»، ردپای Workspace را واقعاً پاک کن | OC-13، §13 |
| BR-9 | **قبل از اعلامِ «یافت‌نشدنی/غیرقابل‌تعمیر»، سه شاهد بیاور.** (۱) مدلِ معماری را از SESSION_STATE/baseline بازگو کن؛ (۲) مکانِ دقیق را از structure baseline بیاب؛ (۳) وجودِ فیزیکیِ فایل را راستی‌آزمایی کن. بدونِ این سه، نتیجه‌گیری ممنوع است | OC-14، §3 |
| BR-10 | **وقتی سند می‌گوید مالک می‌تواند ورودی‌ای بدهد، همان‌جا صریح درخواست کن.** اگر در اسناد خوانده‌شده آمده که مالک «می‌تواند/باید» توکن، آدرس، API key یا تأییدی فراهم کند، Agent باید در همان پیام صریح و کامل درخواست کند و منتظر دستور نماند — سکوت بعد از «خواندم» یعنی کارِ ناقص. | جلسهٔ ۲۰۲۶-۰۸-۲۸ |
| BR-11 | **خالی‌بودن `content/n8n-archive/` ≠ خالی‌بودن آرشیو n8n.** برای وضعیت زندهٔ n8n (workflow، Data Table، تأیید/آرشیو مقاله، execution، تنظیمات) منبع حقیقت، اینستنس زندهٔ SOURCE n8n است (AGENTS.md §2.4). بدون دسترسی read-only: توقف پیش از نتیجه‌گیری + درخواست دسترسی از کاربر (BR-10) — نه استنتاج از پوشهٔ خالی. استفاده از Snapshot فقط در سه شرایط §2.4؛ بازرسی زنده کاملاً read-only | جلسهٔ ۲۰۲۶-۰۸-۲۸ |
