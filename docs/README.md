...
- 🎨 **قالب منتخب ایمیل در کد آماده، هنوز Deploy نشده:** انتخاب مالک `Midnight Gold Classic + Gold Outline` در `api/src/Core/EmailTemplate.php` پیاده‌سازی شد (`4ef2432`): کارت محتوای table-based با قاب طلایی دولایه، هدر/فوتر ثابت، CTA اصلی بدون لینک کمکی و لوگوی واقعی PNG در `public/assets/velora-email-logo.png`. تست قرارداد قالب `21/21` و Resend mock `26/26` سبز است. طبق انتخاب «فقط پیاده‌سازی کد»، Staging و Production تغییر نکرده‌اند و Actions غیرفعال مانده است.
