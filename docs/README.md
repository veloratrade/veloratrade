...
- ⚠️ **تعارض‌های کشف‌شده با وضعیت زنده (گزارش، نه تفسیر):** ① Secrets تولید `FTP_SERVER/USERNAME/PASSWORD` وجود دارند، درحالی‌که §3 می‌گوید «هیچ — عمدی» ② `deploy.yml` دارای اجرای موفق `workflow_dispatch` است (۲۰۲۶-۰۸-۱۷ 20:18)، درحالی‌که OC-9 از ۲۷ اجرای شکست‌خورده می‌گوید ③ دو workflow مستندنشده: `user-journey.yml` و `frontend-url-guard.yml`.
...
| B-11 | **پوشش خودکار گام ۱–۴ سفر (register/verify-email)** — باگ ۵۰۰ ثبت‌نام (رفع‌شده در `08188cb`) دقیقاً از همین شکاف پوشش عبور کرد؛ چالش: گرفتن توکن تأیید بدون خواندن inbox | P1 | ⏳ پیشنهاد ۲۰۲۶-۰۸-۱۸ |
