...
- 🌍 **سخت‌سازی i18n برای توسعهٔ آینده روی شاخهٔ کاری Agent اعمال شد (بدون تغییر Production/Staging و بدون اجرای Actions):** `CLAUDE.md` ریشه به‌عنوان قرارداد مشارکت‌کننده افزوده شد؛ همان validatorهای موجودِ `report_catalog_anomalies.py` و `report_orphan_catalog_keys.py` با allowlist دقیق baseline به حالت blocking ارتقا یافتند تا CI روی کلیدهای orphan جدید، ترجمهٔ EN خالیِ جدید، و `FA==EN`های ترجمه‌نشده fail کند، درحالی‌که parity/placeholder mismatch/hardcoded UI از قبل blocking بودند. استثناهای فعلی به‌صورت exact-key در `tools/localization/catalog-quality-allowlist.json` ثبت شدند و stale allowlist نیز خودش fail می‌شود. این تغییر فقط hardening است؛ معماری i18n موجود refactor نشد.
