...
| ۴ | نبودِ Secret های `FTP_*` | ✅ فعال (عملی، نه طراحی‌شده) | خطای امن است نه گارد — OC-9 |
...
| B-9 | **گارد واقعی تولید:** ① branch policy فقط `main` → ✅ **انجام شد ۲۰۲۶-۰۸-۱۷** ② Required Reviewers → 🔴 **مسدود توسط پلن** (OC-8) — نیازمند ارتقا به GitHub Pro/Team یا عمومی‌کردن مخزن؛ تصمیم مالک ③ تصمیم درباره تریگر `push` در `deploy.yml` (OC-9) — دست‌نخورده باقی ماند ④ اصلاح کامنت گمراه‌کننده `deploy.yml` | 🔴 P0 | ⏳ بندهای ②③④ باز |
