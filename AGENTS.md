...
8. اگر در اسناد خوانده‌شده آمده است که مالک «می‌تواند» یا «باید» ورودی‌ای فراهم کند (توکن، آدرس، API key، تأیید)، همان‌جا در همان پیام صریح و کامل درخواست کن و منتظر دستور نمان (BR-10).
...
- [ ] تمام اسناد n8n (`docs/N8N_*.md`) کامل خوانده شد و وضعیت آرشیو `content/n8n-archive/` بررسی شد؛ در صورت نیاز به بازرسی زنده، ورودی لازم (Base URL / کلید read-only) طبق BR-10 صریح درخواست شد. خالی‌بودن پوشهٔ snapshot به‌معنی خالی‌بودن آرشیو واقعی n8n نیست (§2.4).
...
**قاعده:** کد `VELORA-RUN-*` گروگانِ خواندن کامل است. چاپ آن یعنی گواهی «همه را کامل خواندم و ردپای Workspace حداقلی است». چاپ کد با خواندن ناقص، ادعای بدون شاهد و نقض BR-6 است. «کافی بودنِ» خواندن، تفسیرپذیر نیست — کامل یعنی کامل (BR-1). هر نقض بند ۱۳.۱ در همین جلسه — حتی اگر همان لحظه حذف و گزارش شده باشد — به‌معنای Bootstrap ناموفق است؛ در آن صورت کد چاپ نشود و فقط قالب «Silent Bootstrap ناموفق بود: <دلیل>» بیاید. کدِ موفقیت یعنی «هیچ نقضی رخ نداده»، نه «ردپای فعلی تمیز است».
...
### 2.2.1 بازخوانی و طبقه‌بندی فایل‌های آرشیوی (Archive Reading & Classification)

این قاعده درباره‌ی برخورد با هر فایل/محتویِ آرشیوی در مخزن است — فراتر از آرشیو مقالات n8nِ بند 2.2.

**اصل بنیادی — آرشیو شاهد است، نه مرجع (Archive is evidence, not authority):** محتوای آرشیوی شواهدِ تاریخی است و هرگز به‌تنهایی به‌عنوان مجوزِ جاری یا حقیقتِ جاریِ سیستم در نظر گرفته نمی‌شود.

- فایل‌های آرشیوی منبعِ داده‌ی پایدار (persistent source material) هستند و پیش از هر استفاده باید بازرسی شوند.
- Agent باید محتوای واقعیِ فایل آرشیوی را بخواند و نباید فقط از روی نامِ فایل حدس بزند که چیست.
- اگر محتوای آرشیوی مقاله، پیش‌نویس مقاله، مقاله پژوهشی، مطالب وبلاگ یا هر محتوای مرتبط با مقاله باشد، باید به همین صورت طبقه‌بندی شود.
- محتوای مقاله باید طبق گردش‌کار مقاله/محتوای پروژه و قواعد محتواییِ موجود پردازش شود.
- صرفِ «کشف شدن»ِ محتوای آرشیوی مجوزِ تغییر خودکار، حذف، مهاجرت، انتشار یا تغییر کاربریِ آن نیست.
- این تمایز باید حفظ شود:
  1. منبعِ آرشیوی/سورس (archive/source material)
  2. پیش‌نویس‌های مقاله/محتوا (article/content drafts)
  3. محتوای تولیدشده/منتشرشده (production/published content)
- هرگز آرشیو را مجوزی برای انتشار یا تغییرِ محتوایش ندان.
- اگر آرشیو حاوی محتوای حساس یا secret باشد، قواعد مدیریت secretِ موجود (§8) را رعایت کن و هرگز آن را نادرست افشا یا persist نکن.

**اولویت منابع در تعارض:** منابعِ زنده/معتبرِ جاری بر آرشیوهای تاریخی اولویت دارند. محتوای آرشیو فقط «آنچه در زمان ثبت وجود داشت» را توصیف می‌کند. اگر آرشیو، GitHub، n8n و state تولید با هم تعارض داشتند، سیاستِ منابع حقیقتِ موجود در §11 را دنبال کن و هرگز از یک snapshot قدیمی برای بازنویسیِ state معتبرِ جاری استفاده نکن.

**مدیریت ابهام:** اگر طبقه‌بندی قطعی نیست، حدس نزن؛ محتوا را بازرسی کن. اگر محتوای آرشیو با state معتبرِ جاری در تعارض بود، تعارض را گزارش کن. اگر ابهام می‌تواند به یک عملیاتِ WRITE منجر شود، **توقف کن** و پیش از ادامه، مجوزِ صریحِ مالک را بگیر (طبق قواعد مجوزِ عملیات در §9). هرگز ابهام را به انتشار، مهاجرت، حذف، فعال‌سازی یا تغییر تولیدِ خودکار تبدیل نکن.

**یکپارچگی آرشیو:** snapshotهای ناقص، خراب، ناقص‌مانده، غیرقابل‌تفسیر یا با hash نامطابق را به‌عنوان معتبر پردازش نکن. مشکلِ یکپارچگی را گزارش کن و پردازشِ آن مصداق را تا رفع مشکل متوقف کن.

**طبقه‌بندی و مسیریابی:** محتوای آرشیوی را بر اساس محتوای واقعی به یکی از این نوع‌ها نسبت بده و به سیاستِ حاکمِ مربوطه مسیریابی کن:

- مقاله / پیش‌نویس / محتوای منتشرشده → گردش‌کار مقاله/آرشیوِ همین سیاست
- n8n workflow / اطلاعات Data Table → سیاست مهاجرتِ §2.3
- پیکربندی (configuration) → قواعد مربوط به کانفیگِ مخزن
- مستنداتِ نامرتبط → پردازش‌شده به‌عنوان مرجع، بدون اقدامِ خودکار
- ناشناخته/مبهم → بدون اقدامِ خودکار؛ گزارش کن

مطالب ناشناخته یا نامرتبط نباید هیچ اقدامِ خودکاری را آغاز کنند.

**ادم‌پذیری / جلوگیری از پردازش تکراری:** جایی که در دسترس است از شناسه‌های پایدار/checksumها مانند content SHA-256 استفاده کن. محتوای آرشیویِ یکسان را مکرراً پردازش یا بازنشر نکن. سابقه‌ی پردازشِ قبلی به‌خودی‌خود مجوزِ یک عملیاتِ WRITE جدید نیست. جایی که قابل‌اعمال است برای سوابق آرشیوی ثبت کن: منبع، captured_at/نسخه، content_sha256، طبقه‌بندی/تصمیم، و اقدامِ نتیجه‌شده.

- برای تحلیلِ صرفاً خواندنی (read-only)، وقتی تعارضِ مادی وجود ندارد، از قوی‌ترین قاعده‌ی به‌وضوحِ قابل‌اعمالِ موجود استفاده کن. اما در هر موردِ ابهامِ مادی — به‌ویژه هر چیزی که می‌تواند به WRITE، مهاجرت، انتشار، حذف، فعال‌سازی، استقرار یا تغییرِ تولید منجر شود — **توقف کن**، ابهام را گزارش کن و منتظر مجوزِ صریحِ مالک بمان. هرگز ابهامِ مادی را با حدس حل نکن.

**الزامِ اجرایی (Archive Read Enforcement):** کارِ وابسته به آرشیو (تحلیلِ مبتنی بر آرشیو) مستلزمِ **خواندنِ واقعیِ** فایلِ آرشیویِ canonical است — نه حدس از نام، فهرست‌بندی، یا صرفِ checksum. یک WRITEِ محافظت‌شده که مادیاً به محتوای آرشیو وابسته است (انتشار/مهاجرت/تغییر/حذف/فعال‌سازی/استقرار/تغییرِ تولید) مستلزمِ **شاهدِ قابلِ‌راستی‌آزماییِ** `ARCHIVE_READ_EVIDENCE` از `tools/n8n_archive/read_guard.py` است؛ بدونِ شاهدِ معتبر → FAIL. شاهدِ خواندن **مجوزِ نوشتن نیست**؛ قواعدِ مجوزِ مالک (§2.3، §9) مستقل و پابرجا می‌مانند. در هر ابهام — توقف کن و منتظرِ مالک بمان.
...
## 2.4 منبع حقیقت وضعیت زندهٔ n8n (n8n Live Source of Truth)

قانون صریح و بدون ابهام — برای تمام وظایفی که به Workflowهای n8n، Data Tableهای n8n، وضعیت تأیید یا آرشیو مقالات، Executionهای n8n، تنظیمات n8n یا هر وضعیت زندهٔ دیگر در n8n مربوط می‌شوند:

**اینستنس زندهٔ SOURCE n8n منبع اصلی و معتبر اطلاعات (Source of Truth) است.**

- `content/n8n-archive/` فقط یک Snapshot اکسپورت‌شده در GitHub است و نباید به‌عنوان وضعیت واقعی و زندهٔ n8n در نظر گرفته شود.
- **خالی بودن یا نبودن `content/n8n-archive/` هرگز نباید به‌عنوان مدرکی برای خالی بودن آرشیو واقعی n8n تفسیر شود.** نتیجه‌گیری «آرشیوی وجود ندارد» از روی پوشهٔ خالی ممنوع است.
- اگر برای انجام یک کار اطلاعات زندهٔ n8n لازم است و Agent به SOURCE n8n دسترسی ندارد، Agent باید **پیش از هرگونه نتیجه‌گیری** متوقف شود و طبق BR-10 دسترسی read-only لازم (Base URL + کلید read-only) را صریح از کاربر درخواست کند.
- Agent نباید بدون اطلاع کاربر Snapshot موجود در GitHub را جایگزین اطلاعات زندهٔ n8n کند.

**اولویت منابع برای دامنهٔ n8n:**

```text
Live n8n SOURCE ⬇ GitHub Snapshot ⬇ هیچ فرضی نکن؛ از کاربر دسترسی/اطلاعات لازم را بخواه
```

اگر GitHub خالی است ولی n8n هنوز بررسی نشده، Agent حق ندارد بگوید «آرشیوی وجود ندارد»؛ پاسخ مجاز: «Snapshot گیت خالی است، اما برای تعیین وضعیت واقعی آرشیو باید SOURCE n8n بررسی شود؛ من به دسترسی read-only نیاز دارم.»

**Snapshotهای GitHub فقط در یکی از این سه شرایط قابل استفاده‌اند:**

1. کاربر صراحتاً درخواست بررسی Snapshot را داده باشد؛
2. نیاز به مقایسهٔ تاریخی وجود داشته باشد؛
3. قرارداد پروژه برای همان وظیفه مشخصاً Snapshot را منبع معتبر معرفی کرده باشد (مانند گیت صلاحیت پردازش مقاله در §2.2 و `docs/N8N_ARCHIVE_AGENT.md`؛ آن گیت snapshot-محور باقی می‌ماند و این قاعده آن را تغییر نمی‌دهد).

**محدودهٔ امنیتی — فقط READ:** این قانون هیچ مجوز جدیدی ایجاد نمی‌کند. تا مجوز صریح و جداگانهٔ مالک، بازرسی زندهٔ n8n کاملاً read-only می‌ماند: تغییر Workflow، فعال/غیرفعال‌سازی، انتشار مقاله، تغییر Data Table/Credential/تنظیمات، ایجاد یا حذف منابع، Deploy و هر عملیات Write ممنوع است (§2.3.5، §9).
...
برای دامنهٔ n8n، منبع حقیقت وضعیت زنده، اینستنس زندهٔ SOURCE n8n است (§2.4)؛ Snapshotهای GitHub فقط در سه شرایط §2.4 معتبرند.
...
- «Clone کامل» شامل **هر** شکلی از واکشیِ کل درخت است — کلون عمیق git، دانلود tarball/zip از UI یا API، یا اکسترکت کامل — نه فقط دستور `git clone`. فقط واکشی تک‌فایل (raw/API) یا sparse-checkout محدود به مأموریت مجاز است.
...
## 14. قانون دائمی BACKUP GATE (الزامی برای عامل هوش مصنوعی — ۲۰۲۶-۰۹-۰۹)

هر زمان که درخواست کاربر شامل یکی از موارد زیر باشد:

- انتقال/سینک دیتابیس (staging یا production)
- اجرای مهاجرت دیتابیس (migration)
- هر تغییر دیتابیسی در Staging یا Production
- استقراری که تغییرات دیتابسی همراه دارد

عامل **مكلف** است ابتدا BACKUP GATE را با شواهد ماشینی راستی‌آزمایی کند:

- خروجی‌های workflow به‌روزِ `velora-db-backup-staging.yml` باید موجود و معتبر باشند:
  `backup_id` (پیشوند `db-backup-staging-`)، `release_tag`، `sha256` (۶۴ هگز)،
  `source_commit_sha`، `verification_status == INTEGRITY_VERIFIED`، `environment`.
- اعتبارسنجی فقط از طریق `ops/velora-mgmt/backup_gate.py` یا خروجی‌های قابل
  استناد run گیتاب انجام می‌شود.
...
«سیستم بکاپ داریم»، پیام لاگ، حدس، بکاپ قدیمی یا timestamp به‌هیچ‌وجه معادل
