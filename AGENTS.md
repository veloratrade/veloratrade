...
- PAT داخل چت باید افشاشده فرض و پس از جلسه Revoke شود.
...
- گارد خودکار: `tools/velora-status.sh` ردپای Workspace را می‌سنجد و نقض را با `WORKSPACE-FOOTPRINT-VIOLATION` در بخش Drift گزارش می‌کند (حالت `--check` را هم قرمز می‌کند).
